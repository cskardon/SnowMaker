...
namespace SnowMaker
{
    public class UniqueIdGeneratorAsync : IUniqueIdGenerator
    {
        readonly IOptimisticDataStoreAsync optimisticDataStore;
        ...
        private void FetchFromSyncStore(string scopeName, ScopeState state, int batchSize)
        {
            if (state.PrefetchTask != null) {
                ...
                UpdateFromSyncStore(scopeName, state, batchSize).Wait();
            }
        ...
        async Task UpdateFromSyncStore(string scopeName, ScopeState state, int batchSize)
        {
            ...
            while (writesAttempted < maxWriteAttempts) {
                ...
                if (await optimisticDataStore.TryOptimisticWriteAsync(scopeName, firstIdInNextBatch.ToString(CultureInfo.InvariantCulture)))
                    return;
